...
async function loadModels() {
  ...
  try {
    ...
    if (res.error) { select.innerHTML = `<option value="">(接続エラー)</option>`; return; }
...
async function loadTemplates() {
  ...
  try {
    ...
    select.innerHTML = `<option value="">一般返信</option>`;
  }
}
...
async function loadHistory() {
  ...
  try {
    ...
    list.innerHTML = history.map((h) => `
      <div class="history-item" data-reply="${encodeURIComponent(h.reply)}">
        <div class="h-subject">${escapeHtml(h.subject)}</div>
        <div class="h-meta">
          ${new Date(h.timestamp).toLocaleString("ja-JP")} ・
          ${h.model} ・ ${h.tone} ・ ${h.language}
        </div>
        <div class="h-preview">${escapeHtml(h.reply.substring(0, 100))}</div>
      </div>
    `).join("");

    list.querySelectorAll(".history-item").forEach((item) => {
      item.addEventListener("click", () => {
        const reply = decodeURIComponent(item.dataset.reply);
...
function renderTemplateEditor(templates) {
  ...
  list.innerHTML = templates.map((t, i) => `
    <div class="tpl-item" data-idx="${i}">
      <input class="tpl-name" value="${escapeHtml(t.name)}" placeholder="名前">
      <input class="tpl-prompt" value="${escapeHtml(t.prompt)}" placeholder="プロンプト（空=一般返信）">
      <button class="btn btn-secondary btn-sm tpl-delete" data-idx="${i}">×</button>
    </div>
  `).join("");
